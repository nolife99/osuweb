define(["underscore", "osu-audio", "curve/LinearBezier", "curve/ArcPath"], (_, OsuAudio, LinearBezier, ArcPath) => {
                ...
                switch (section) {
                    ...
                    case "[HitObjects]":
                        ...
                        if ((hit.type & HIT_TYPE_CIRCLE) > 0) {
                            ...
                            for (var wdnmd = 0; wdnmd < hit.repeat + 1; ++wdnmd) hit.edgeSets.push({
                                normalSet: 0,
                                additionSet: 0
                            });
                            if (parts.length > 9) {
    ...
    function Osu(zip) {
        ...
        this.load = function() {
            self.raw_tracks = _.filter(self.zip.children, c => c.name.indexOf(".osu") === c.name.length - 4);
        ...
        this.getCoverSrc = function(img) {
            for (let i = 0; i < this.tracks.length; ++i) {
                ...
                try {
                    ...
                    this.zip.getChildByName(file).getBlob("image/jpeg", blob => img.src = URL.createObjectURL(blob));
                    break;
                }
                catch (error) {
        ...
        this.filterTracks = function() {
            self.tracks = self.tracks.filter(t => t.general.Mode !== 3);
        }
        this.sortTracks = function() {
            self.tracks.sort((a, b) => a.difficulty.OverallDifficulty - b.difficulty.OverallDifficulty);
        }
        this.load_mp3 = function load_mp3() {
            _.find(self.zip.children, function(c) {
                return c.name.toLowerCase() === self.tracks[0].general.AudioFilename.toLowerCase();
            }).getBlob("audio/mpeg", function(blob) {
